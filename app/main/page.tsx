    ...
    console.log("Image deleted");
      ...
      const generateIcon = async () => {
        ...
        try {
          ...
          console.log('Sending generate request with prompt:', prompt);
          ...
          if (response.ok && data.success && data.predictionId) {
            await updateUserPoints(newPoints); // Update points in database
      ...
      const pollForResult = async (predictionId: string) => {
        ...
        const checkResult = async () => {
          ...
          try {
            console.log(`Checking prediction (Attempt ${attempts}):`, predictionId) // Add this log
            ...
            console.log('Received check-prediction response:', data) // Add this log
      
            if (data.status === 'completed') {
              setOriginalImageUrl(data.imageUrl)
              console.log('Image generation completed. URL:', data.imageUrl) // Add this log
            } else if (data.status === 'failed') {
              setError('Failed to generate icon. Please try again.')
            } else {
              console.log('Generation still in progress. Checking again in 5 seconds.') // Add this log
              setTimeout(checkResult, 5000) // Check again after 5 seconds
            }
          } catch (error) {
      ...
      Size of the generated image in text-to-image mode. Only provides 256,512,1024
            ...
            <div className="flex-1 p-5 md:p-[30px] md:py-14 flex flex-col justify-start items-start">
              <div className="w-full pr-0 md:pr-5">
                ...
                <div className="w-full h-[300px] md:h-[570px] flex justify-center items-center mb-5">
                  {((showOriginal && originalImageUrl) || (!showOriginal && removedBgImageUrl)) ? (
                    <Image 
                    src={showOriginal ? originalImageUrl || '/placeholder.png' : removedBgImageUrl || '/placeholder.png'}
                    alt="Generated Icon"
